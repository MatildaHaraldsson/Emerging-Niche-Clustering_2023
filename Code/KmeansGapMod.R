...
## Modifications made to script to extract additional information from the kmeans() calculations (H&T)
## 1) Log+1 transforms the the normalized abundance data. Speeds up the calculations considerably.
## 2) Cluster identity per trait : 'clusterID' matrix with normalised abundances, rescaled traits, and cluster identity for each No of Clusters (kvec)
## 3) Sum of squares within clusters (withinss) :  'Wk.clust' list of vectors, one value / cluster per No of Clusters (kvec)
## 4) Sum of squares between clusters (betweenss) : 'Bwk' vector, one value per No of Clusters (kvec)
## NOTE: these modifications are only applicable on the core settings of KmeansGap() in cluster analysis 
## e.g. multiD = FALSE, ...

KmeansGapMod=function(
  dat,
  nozeros=FALSE,
  multiD=FALSE,
  nullmodel='shuffle',
  ...
  #min.dat=min(1,min(dat$N[dat$N>0]))      # H&T - save minimum abundance for renormailization of data
  ...
  dat$N=log(dat$N+1)                      # H&T - speed up time by log+1 transforming
  ...
  rownames(traitdata)=rownames(dat)   # H&T - preserve original rownames (species identify)
  ...
  ## H&T - modify function to ensure rownames (species identity) are preserved
  Com=function(traits,abuns) {
    if(ncol(traits)>1) rep.row(r=traits,a=ceiling(abuns)) else 
      com1=rep(traits$trait,ceiling(abuns)) # H&T - ceiling() function added to be sure values < 1 are included
    com1=as.matrix(com1)
    rownames(com1)=rep(rownames(traits),ceiling(abuns))
    return(com1)
  }
  ...
  ## H&T - modified to extract additional cluster information (Bwk, Wk.clust, and clusterID)
  ## perform the kmeans test on the observed community
  ...
  com.abuns=rep.row(as.data.frame(dat$N),ceiling(dat$N))
  com.abuns=as.matrix(com.abuns)
  rownames(com.abuns)=rep(rownames(traitdata),ceiling(dat$N))
  clusterID1=cbind(N=com.abuns,mu=Com(traitdata,dat$N))
  colnames(clusterID1[,c(1:2)])=c("N","mu")
  clusterID=clusterID1[unique(rownames(clusterID1)),]
  rm(clusterID1,com.abuns)
  Wk<-NULL
  Bwk<-NULL
  Wk.clust<-list()
  
  # H&T. A general test to avoid error message - number of potential clusters identified cannot be larger than the number of individuals that survived
  comTest=Com(traitdata,dat$N)
  nrTest <- length(unique(rownames(comTest)))
  if(nrTest < tail(kvec,n=1)) kvec <- c(1:nrTest)
  # end of inclusion
  ...
  for (krep in kvec){
    ...
    mod=kms(com,krep)
    rm(com)
    Wk=c(Wk,Dispersion(dtf=mod,weight=weighting))
    Bwk=c(Bwk,mod$betweenss)
    Wk.clust[[krep]]<-mod$withinss
    clusterID1=mod$cluster
    com.abuns=rep.row(as.data.frame(dat$N),ceiling(dat$N))
    com.abuns=as.matrix(com.abuns)
    rownames(com.abuns)=rep(rownames(traitdata),ceiling(dat$N))
    clusterID1=cbind(N=com.abuns,clusterID1)
    rm(com.abuns)
    clusterID=cbind(clusterID,clusterID1[unique(rownames(clusterID1)),2])
    rm(mod,clusterID1)
  }

  ## H&T - control calculation if clusterID abundances and trait values agrees with original (formatted) data
  ## if not, an error message is generated
  if(! sum(colSums(clusterID[,c(1:2)])) == sum(c(sum(dat$N),sum(traitdata))) ) stop('clusterID dataframe not correctly coerced')
  ...
  return(list(
    ...
    weighting=weighting,
    dispersion=disp,
    Wk.clust=Wk.clust, # H&T added - (vector of withinss/cluster for optimal cluster)
    clusterID=clusterID  # H&T added - dataframe (or vector) of cluster identity for each individual here!
  ))
}
...
getClusterStat = function(trait,N,K) {
  # trait   = trait values
  # N       = abundances
  # centers = number of optimcal clusters
  # nstart  = nstarting points from KmeansGap() settings, typically = 100
